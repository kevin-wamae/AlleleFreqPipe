...
library(data.table) # reading in data using fread
...
freqStats_v1 <- data.frame(n = as.numeric(),
                        freq = as.numeric(),
                        base = as.character(),
                        locus = as.character())
...
snpFreq <- freqStats_v1 %>% mutate(locus = gsub("V", "", locus), #replace "V" with blanks
